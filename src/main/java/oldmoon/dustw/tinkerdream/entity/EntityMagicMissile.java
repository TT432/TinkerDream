    ...
    public void onUpdate() {
        ...
        if(world.isRemote){
            for(int i=0; i<5; i++){
                ...
                Vec3d particlesPosVec = this.getPositionVector().add(dx - this.motionX / 2, dy - motionY / 2, dz - this.motionZ / 2);
                ...
                if(ticksExisted > 1){
                    ...
                    particlesPosVec = this.getPositionVector().add(dx - this.motionX, dy - motionY, dz - this.motionZ);
        ...
        if(hit != null){
            if(AllyDesignationSystem.isValidTarget(getThrower(), hit)){
                ...
                TinkerDream.logger.info(String.format("new Motion %s", newMotion));
